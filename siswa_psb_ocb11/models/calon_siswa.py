...
class calon_siswa(models.Model):
    ...
    state = fields.Selection([('draft', 'Draft'), ('reg', 'Registered')],
                             string='State', required=True, default='draft')
    is_siswa_lama = fields.Boolean('Siswa Lama', default=False)
    siswa_id = fields.Many2one(
        'res.partner', string="Data Siswa Lama", ondelete="restrict")
    prev_rombel_id = fields.Many2one(
        'siswa_ocb11.rombel', related="siswa_id.active_rombel_id", string="Jenjang Asal")
    no_induk = fields.Char('Siswa ID', related="siswa_id.induk")
    registered_siswa_id = fields.Many2one(
        'res.partner', string="Registered Siswa")
    tahunajaran_id = fields.Many2one('siswa_ocb11.tahunajaran', string='Tahun Ajaran', required=True,
                                     default=lambda self: self.env['siswa_ocb11.tahunajaran'].search([('active', '=', True)]))
    ...
    jenjang_id = fields.Many2one(
        'siswa_ocb11.jenjang', string='Jenjang', required=True)
    tanggal_registrasi = fields.Date(
        'Tanggal Registrasi', required=True, default=datetime.today().date())
    tahunajaran_id = fields.Many2one(
        'siswa_ocb11.tahunajaran', string='Tahun Diterima')
    induk = fields.Char(string='Internal Reference',
                        required=False, copy=False, readonly=True, default="New")
    nis = fields.Char(string="NIS", required=False)
    panggilan = fields.Char(string="Panggilan")
    jenis_kelamin = fields.Selection(
        [('laki', 'Laki-laki'), ('perempuan', 'Perempuan')], string='Jenis Kelamin', required=True)
    ...
    rombels = fields.One2many('siswa_ocb11.rombel_siswa',
                              inverse_name='siswa_id', string='Detail Rombongan Belajar')
    ...
    non_aktif_selection = fields.Selection(
        [('mutasi', 'Mutasi'), ('lulus', 'Lulus'), ('meninggal', 'Meninggal Dunia')], string='Keterangan')
    ...
    rombel_id = fields.Many2one(
        'siswa_ocb11.rombel', string='Rombongan Belajar', domain="[('jenjang_id','=',jenjang_id)]")
...
#                         string="Biaya Opsional", ondelete="cascade")
    ...
    def calculate_bayar_tunai(self):
        ...
        total_harus_bayar = sum(
            map(lambda x: x.jumlah_harga_setelah_potongan, self.pembayaran_lines))
        tunai = self.bayar_tunai
        if tunai > total_harus_bayar:
            raise ValidationError(
                _('Jumlah bayar tunai melebihi jumlah tagihan.'))
        else:
    ...
    def create(self, vals):
        if vals.get('reg_number', _('New')) == _('New'):
            if 'company_id' in vals:
                vals['reg_number'] = self.env['ir.sequence'].with_context(
                    force_company=vals['company_id']).next_by_code('reg.no.siswa.psb.ocb11') or _('New')
            else:
                vals['reg_number'] = self.env['ir.sequence'].next_by_code(
                    'reg.no.siswa.psb.ocb11') or _('New')
    ...
    def recompute_psb_dashboard(self, tahun=False):
        ...
        # if tahun:
        #     psb_dash = self.env['psb_dashboard'].search(
        #         [('tahunajaran_id', '=', tahun)])
        # else:
        #     psb_dash = self.env['psb_dashboard'].search(
        ...
        psb_dash = self.env['psb_dashboard'].search([])
        for dash in psb_dash:
            dash.regenerate_dashboard()
    ...
    def assign_biaya_line_to_siswa(self, siswa):
        ...
        res_partner_siswa = self.env['res.partner'].search(
            [('id', '=', siswa.id)])
    ...
    def action_confirm(self):
        ...
        if len(self.pembayaran_lines) > 0:
            total_harus_bayar = sum(
                map(lambda x: x.jumlah_harga_setelah_potongan, self.pembayaran_lines))
            dibayar_on_lines = sum(
                map(lambda x: x.dibayar, self.pembayaran_lines))
            tunai = self.bayar_tunai
            ...
            if tunai > total_harus_bayar:
                ...
                raise ValidationError(
                    _('Jumlah bayar tidak sesuai dengan jumlah tagihan.'))
            ...
            raise exceptions.except_orm(_('Warning'), _(
                'Can not confirm this registration, complete payment first!'))
    ...
    def action_reset(self):
        ...
        if not self.is_siswa_lama:
            ...
            self.env.cr.execute(
                'select id from siswa_ocb11_tahunajaran where sort_order = ' + str(self.tahunajaran_id.sort_order - 1))
...
#         biaya_ids = []
    ...
    def generate_pembayaran_lines(self):
        ...
        #
            ...
            raise exceptions.except_orm(_('Warning'), _(
                'Data biaya belum terisi, lengkapi dulu pada tab Biaya'))
...
#         self.bayar_tunai = sum(map(lambda x: x.harga, self.pembayaran_lines))
    ...
    def unlink(self):
        ...
        # res.recompute_psb_dashboard(tahun_id)
        # recompute dashboard
        psb_dash = self.env['psb_dashboard'].search([])
        for dash in psb_dash:
            dash.regenerate_dashboard()
    ...
    @api.multi
    def write(self, vals):
        res = super(calon_siswa, self).write(vals)
