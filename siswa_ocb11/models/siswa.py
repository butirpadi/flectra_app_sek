...
class siswa(models.Model):
    ...
    tahunajaran_id = fields.Many2one(
        'siswa_ocb11.tahunajaran', string='Tahun Diterima', required=False, ondelete="restrict")
    induk = fields.Char(string='Siswa ID', required=False,
    ...
    jenis_kelamin = fields.Selection(
        [('laki', 'Laki-laki'), ('perempuan', 'Perempuan')], string='Jenis Kelamin', required=False)
    ...
    active_rombel_id = fields.Many2one(
        'siswa_ocb11.rombel', string='Rombongan Belajar', compute='_compute_rombel', store=True)
    ...
    def create(self, vals):
        if vals.get('induk', _('New')) == _('New'):
            if 'company_id' in vals:
                vals['induk'] = self.env['ir.sequence'].with_context(
                    force_company=vals['company_id']).next_by_code('siswa.ocb11') or _('New')
            else:
                vals['induk'] = self.env['ir.sequence'].next_by_code(
                    'siswa.ocb11') or _('New')
...
#         self.value2 = float(self.value) / 100
    ...
    def set_default_domain_user(self):
        ...
        self.env.cr.execute(
            "update res_partner set tz = 'Asia/Jakarta' where id = " + str(root_user[0].partner_id.id))
    ...
    def toggle_active(self):
        ...
        if self.active:
            ...
            self.env.cr.execute("update siswa_ocb11_rombel_siswa set active = 'True' where siswa_id = '" +
                                str(self.id) + "' and rombel_id = " + str(self.active_rombel_id.id))
