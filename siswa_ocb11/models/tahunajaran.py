...
class tahunajaran(models.Model):
    ...
    rombels = fields.One2many(
        'siswa_ocb11.rombel_siswa', inverse_name='tahunajaran_id')
    jenjangs = fields.One2many('siswa_ocb11.tahunajaran_jenjang',
                               inverse_name='tahunajaran_id', string='Jenjang', ondelete='cascade')
            ...
            if 'active' in values:
                if values['active']:
                    is_super_user = self.env.user.has_group(
                        'base.group_system')
                    if is_super_user:
                        rec.env['siswa_ocb11.tahunajaran'].search(
                            [('company_id', '=', rec.company_id.id), ('id', '!=', rec.id)]).write({'active': False})
                    else:
                        rec.env['siswa_ocb11.tahunajaran'].search(
                            [('company_id', '=', self.env.user.company_id.id), ('id', '!=', rec.id)]).write({'active': False})
    ...
    def create(self, vals):
        ...
        if len(taj) > 0:
            # get max order
            self.env.cr.execute(
                'select max(sort_order) from siswa_ocb11_tahunajaran')
    ...
    # 	# auto generate tahunajaran_jenjang
        jenjangs = self.env['siswa_ocb11.jenjang'].search(
            [('name', 'ilike', '%')])
        print('----------------------------------------')
...
# 			})
        ...
        print('----------------------------------------')
        rombels = self.env['siswa_ocb11.rombel'].search(
            [('name', 'ilike', '%')])
        for rb in rombels:
