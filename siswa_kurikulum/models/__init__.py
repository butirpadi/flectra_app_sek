# -*- coding: utf-8 -*-

# from . import tahunajaran
# from . import hr_job
from . import kurikulum_setting
from . import tema
from . import subtema
from . import protah
from . import protah_tema_rel
from . import prosem
from . import kompetensi_inti
from . import kompetensi_dasar
from . import kategori_muatan_materi
from . import muatan_materi
from . import rppm_setting
from . import rppm_setting_kategori_muatan_rel
from . import rppm
from . import rppm_muatan_materi_base_rel
from . import rppm_muatan_materi_rel