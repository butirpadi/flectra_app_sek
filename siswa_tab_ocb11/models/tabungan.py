...
class tabungan(models.Model):
    ...
    saldo_tabungan = fields.Float(
        'Saldo Tabungan', compute="_compute_get_saldo", store=True)
    ...
    jenis = fields.Selection([('setor', 'Setoran'), ('tarik', 'Tarik Tunai')],
                             string='Jenis', required=True, default='setor')
    confirm_ids = fields.One2many(
        'siswa_tab_ocb11.action_confirm', inverse_name="tabungan_id")
    ...
    def create(self, vals):
        ...
        if can_draw:
            if vals.get('name', _('New')) == _('New'):
                vals['name'] = 'DRAFT/TAB/' + \
                    str(datetime.today().date().strftime('%d%m%y/%H%M%S'))
    ...
    def update_saldo_siswa(self):
        self.ensure_one()
        # update saldo siswa
        tabs = self.env['siswa_tab_ocb11.tabungan'].search(
            [('siswa_id', '=', self.siswa_id.id), ('state', '=', 'post')])
    ...
    def action_confirm(self):
        ...
        name_seq = self.env['ir.sequence'].next_by_code(
            'tabungan.siswa.tab.ocb11') or _('New')
    ...
    def update_saldo_tabungan_dashboard(self):
        ...
        dash_tab = self.env['siswa_tab_ocb11.dashboard_tabungan'].search(
            [('id', 'ilike', '%')])
    ...
    def action_cancel(self):
        ...
        # reset journal entries & delete it
