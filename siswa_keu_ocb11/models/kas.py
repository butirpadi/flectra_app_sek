...
class kas(models.Model):
    ...
    pembayaran_id = fields.Many2one(
        'siswa_keu_ocb11.pembayaran', string='Pembayaran', ondelete='cascade')
    is_related = fields.Boolean('Is Related', default=False)
    state = fields.Selection([('draft', 'Draft'), ('post', 'Posted')],
                             string='State', required=True, default='draft')
    is_allow_to_delete = fields.Boolean('Allow to Delete', default=True)
    kas_kategori_id = fields.Many2one(
        'siswa_keu_ocb11.kas_kategori', required=True, string='Akun Kas', ondelete="restrict")
    ...
    def recompute_keuangan_dashboard(self):
        ...
        dash_keuangan_id = self.env['ir.model.data'].search(
            [('name', '=', 'default_dashboard_kas'), ('company_id', '=', self.company_id.id)]).res_id
    ...
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):
            ...
            if 'company_id' in vals:
                vals['name'] = self.env['ir.sequence'].with_context(
                    force_company=vals['company_id']).next_by_code('siswa.keu.ocb11.kas') or _('New')
            else:
                vals['name'] = self.env['ir.sequence'].next_by_code(
                    'siswa.keu.ocb11.kas') or _('New')
        ...
        # get kas kategori
        kategori_id = self.env['siswa_keu_ocb11.kas_kategori'].search(
            [('id', '=', vals['kas_kategori_id'])])
        ...
        for rec in self:
            if not rec.is_allow_to_delete:
                raise exceptions.except_orm(_('Warning'), _(
                    'You can not delete this type of data.'))

            res = super(kas, self).unlink()
